...
public interface MemberOrganizationRoleRepository
  ...
  @Query(value = """
      ...
      WHERE
          ...
          AND mor.year =CASE
              WHEN EXTRACT(MONTH FROM NOW()) BETWEEN 8 AND 12 THEN date_part('year', NOW())
  ...
  @Query(value = """
      SELECT
          status,
          COUNT(*) * 100.0 / (SELECT COUNT(*) FROM member_organization_role mor WHERE mor.year * 10 + mor.semester >= (:startYear * 10 + :startSemester) AND status = 'active' OR status='inactive') AS percentage
      FROM
        member_organization_role mor
      WHERE
        mor.year * 10 + mor.semester >= (:startYear * 10 + :startSemester) AND status = 'active' OR status='inactive'
      GROUP BY
        status
      """, nativeQuery = true)
  ...
  public interface AvailableAcademicYears {
    Integer getYear();
  }

  @Query(value = """
      SELECT DISTINCT year FROM member_organization_role WHERE organization_id = :organizationId;
      """, nativeQuery = true)
